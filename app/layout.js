...
export default function RootLayout({ children }) {
    const header = (
        <header className={"p-4 sm:p-8 flex items-center justify-between gap-4"}>
            ...
            <div className={`flex items-center justify-between`}>
                <span>PLACEHOLDER CTA || STATS</span>
            </div>
        </header>
    )
  ...
  return (
    <html lang="en">
      <body className={`w-full max-w-[1000px] mx-auto text-sm sm:text-base min-h-screen flex flex-col sample-class text-slate-800 ${opensans.className}`}>
        {header}
        {children}
        {footer}
     </body>
